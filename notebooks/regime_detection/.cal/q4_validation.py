"""Q4 validation outputs (post market concept aggregation + beta-adjusted)."""
...
from market_helper.data_sources.yahoo_finance.market_panel import load_market_panel
from market_helper.regimes.methods.market_regime import (
    compute_market_axis_scores, load_market_regime_config,
)
...
# Market internals
mpanel = load_market_panel("data/interim/market_regime/market_panel.feather")
mcfg = load_market_regime_config("configs/regime_detection/market_regime.yml")
mscores = compute_market_axis_scores(mpanel, mcfg)
mscores = mscores[mscores["date"] >= "2000-01-01"].copy()

# Macro internals (kept stable across rounds)
panel_path = "data/interim/fred/macro_panel.feather"
...
mcfg_engine = load_macro_regime_config("configs/regime_detection/fred_series.yml")
mac = compute_macro_axis_scores(load_panel(panel_path), specs, concepts, config=mcfg_engine)
mac = mac[mac["date"] >= "1995-01-01"].copy()
...
def stats(s):
    ...
    if s.empty:
        return {"mean": np.nan, "std": np.nan, "abs_p95": np.nan, "min": np.nan, "max": np.nan, "n_obs": 0}
    return {"n_obs": int(len(s)), "mean": float(s.mean()), "std": float(s.std()),
            "abs_p95": float(s.abs().quantile(0.95)),
            "min": float(s.min()), "max": float(s.max())}


# (1) Per-signal contribution stats — market layer
rows = []
for col in [c for c in mscores.columns if c.startswith("contrib:")]:
    name = col.replace("contrib:", "")
    sig = next((s for s in mcfg.signals if s.name == name), None)
    if sig is None:
        continue
    rows.append({"signal": name, "axis": sig.axis, "transform": sig.transform, **stats(mscores[col])})
sig_df = pd.DataFrame(rows).sort_values(["axis", "signal"])
sig_df.to_csv(ART / "q4_per_signal.csv", index=False)
print("=== (1) Per-signal contribution stats — market layer (post-Q4) ===")
print(sig_df.to_string(index=False))

# (2) Per-concept stats — market layer
rows = []
for col in [c for c in mscores.columns if c.startswith("concept:")]:
    parts = col.split(":")
    axis, name = parts[1], parts[2]
    matching = next((c for c in mcfg.concepts if c.axis == axis and c.name == name), None)
    rows.append({"axis": axis, "concept": name,
                 "weight": matching.weight if matching else np.nan, **stats(mscores[col])})
con_df = pd.DataFrame(rows).sort_values(["axis", "concept"])
con_df.to_csv(ART / "q4_per_concept.csv", index=False)
print("\n=== (2) Per-concept stats — market layer ===")
...
for axis in ("growth", "inflation"):
    macro_col = pd.to_numeric(mac[axis], errors="coerce")
    rows.append({"axis": axis, "layer": "macro", **stats(macro_col)})
    market_col = pd.to_numeric(mscores[axis], errors="coerce")
    rows.append({"axis": axis, "layer": "market", **stats(market_col)})
    rows.append({"axis": axis, "layer": "final", **stats(results[f"final_{axis}_score"])})
ax_df = pd.DataFrame(rows)
ax_df.to_csv(ART / "q4_axis_dist.csv", index=False)
print("\n=== (3) Per-axis distribution ===")
print(ax_df.to_string(index=False))

# (4) Anchor regime summary
...
anchors.to_csv(ART / "q4_anchors.csv")
print("\n=== (4) Anchor regime summary ===")
...
yr.to_csv(ART / "q4_disagreement_yearly.csv")
...
runs_df.to_csv(ART / "q4_runs.csv", index=False)
print(runs_df["days"].describe(percentiles=[.25, .5, .75, .95]).round(1).to_string())

# Plots
fig, axes = plt.subplots(3, 1, figsize=(13, 9), sharex=True)
for ax_idx, (axis_name, title) in enumerate([
    ("growth", "Growth concepts (post-Q4)"),
    ("inflation", "Inflation concepts (post-Q4)"),
    ("risk", "Risk concepts (post-Q4)"),
]):
    ax = axes[ax_idx]
    cnames = [c.name for c in mcfg.concepts if c.axis == axis_name]
    if axis_name == "risk":
        palette = plt.cm.Reds(np.linspace(0.4, 0.95, len(cnames) or 2))
    elif axis_name == "growth":
        palette = plt.cm.Blues(np.linspace(0.4, 0.95, len(cnames) or 2))
    else:
        palette = plt.cm.viridis(np.linspace(0.1, 0.9, len(cnames) or 2))
    for name, color in zip(cnames, palette):
        col = f"concept:{axis_name}:{name}"
        if col in mscores.columns:
            ax.plot(mscores["date"], mscores[col], lw=0.7, color=color, alpha=0.9, label=name)
    ax.axhline(0, color="black", lw=0.5)
    ax.axhline(0.20, color="grey", ls="--", lw=0.5)
    ax.axhline(-0.20, color="grey", ls="--", lw=0.5)
    ax.set_title(title)
    ax.grid(alpha=0.3); ax.set_ylim(-1.1, 1.1)
    ax.legend(ncol=4, fontsize=8, loc="lower left")
plt.tight_layout(); plt.savefig(ART / "q4_market_concepts_ts.png", dpi=120)
...
for ax, axis_name in [(axes[0], "growth"), (axes[1], "inflation")]:
    ...
    ax.axhline(0.20, color="grey", ls="--", lw=0.5)
    ax.axhline(-0.20, color="grey", ls="--", lw=0.5)
    ax.set_title(f"{axis_name} axis: macro vs market vs final (post-Q4)")
    ax.grid(alpha=0.3); ax.legend(ncol=3, fontsize=8, loc="lower left")
    ax.set_ylim(-1.5, 1.5)
plt.tight_layout(); plt.savefig(ART / "q4_axes.png", dpi=120)

# Regime time-strip
fig, ax = plt.subplots(figsize=(13, 2.6))
res_post = results[results["date"] >= "1995-01-01"].copy()
all_regimes = sorted(res_post["final_regime"].unique().tolist())
palette = plt.cm.tab20.colors
color_map = {r: palette[i % len(palette)] for i, r in enumerate(all_regimes)}
...
ax.set_yticks([]); ax.set_title("Regime time-strip (post-Q4)")
ax.grid(alpha=0.3)
plt.tight_layout(); plt.savefig(ART / "q4_regime_strip.png", dpi=120)
print("\nplots saved.")
