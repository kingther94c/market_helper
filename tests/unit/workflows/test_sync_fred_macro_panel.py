...
def test_resolve_fred_api_key_reads_gdrive_root_local_env(tmp_path, monkeypatch) -> None:
    default_env = tmp_path / "local.env"
    gdrive_root = tmp_path / "005 Portfolio"
    gdrive_root.mkdir()
    synced = gdrive_root / "local.env"
    default_env.write_text('FRED_API_KEY="default-key"\n', encoding="utf-8")
    synced.write_text('FRED_API_KEY="synced-key"\n', encoding="utf-8")
...
def test_resolve_fred_api_key_falls_back_to_default_local_env(tmp_path, monkeypatch) -> None:
    ...
    empty_root.mkdir()  # no local.env inside → falls through to default

    monkeypatch.delenv("FRED_API_KEY", raising=False)
    monkeypatch.setenv("MARKET_HELPER_GDRIVE_ROOT", str(empty_root))
    monkeypatch.setattr(sync_fred_macro_panel, "_DEFAULT_LOCAL_ENV_PATH", default_env)
