...
def rolling_zscore(values: list[float], window: int) -> list[float]:
    ...
    out: list[float] = []
    for value, m, s in zip(values, means, stds):
        if s == 0:
            out.append(0.0)
        else:
...
def compute_factor_snapshots(
    *,
    ...
    vol = [(a + b) / 2.0 for a, b in zip(vix_norm, move_norm)]
    ...
    credit = [0.7 * level + 0.3 * change for level, change in zip(oas_level_norm, oas_change_norm)]
    ...
    rates = [max(-1.0, min(1.0, mean([a, b]) * 10.0)) for a, b in zip(d2, d10)]

    curve_slope = [a - b for a, b in zip(y10, y2)]
    ...
    risk_appetite = [max(-1.0, min(1.0, (eq - fi) * 5.0)) for eq, fi in zip(eq_momo, fi_momo)]
    growth = [
        max(-1.0, min(1.0, 0.5 * ra + 0.3 * cr + 0.2 * sl))
        for ra, cr, sl in zip(risk_appetite, credit_relief, slope_norm)
    ]
    ...
    trend = []
    for a, b, c, d in zip(eq_20, eq_60, fi_20, fi_60):
        score = 0.0
