...
def _carry_tilt_suggestion(alloc, *, as_of: str, data_mode: str) -> Suggestion | None:
    ...
    return Suggestion(
        ...
        why_now="Carry from configured overnight-rate differentials vs USD (indicative, not optimised).",
        headline_metrics={"top": f"{best[0]} {best[1]:+.0f}bps", "bottom": f"{worst[0]} {worst[1]:+.0f}bps"},
        rationale="Annualized carry (bps of leg notional): "
        + ", ".join(f"{c} {b:+.0f}" for c, b, _ in ranked),
        data_mode=data_mode,
        body_kind="fx_carry",
...
class FxHedgeAdvisorPlugin:
    ...
    def produce(
        ...
        *,
        ...
        tilt = _carry_tilt_suggestion(alloc, as_of=run_as_of, data_mode=data_mode)
        if tilt is not None:
