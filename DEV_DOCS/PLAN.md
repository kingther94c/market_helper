...
- Added a lightweight Commodity Sector Summary table in the Commodity detail tab, aggregating CM positions by `cm_sector` (PM / IM / EN / AG) loaded directly from `configs/security_universe.csv`. Full propagation of `cm_sector` through `SecurityReference` and a cross-sector correlation heatmap are deferred to a future change.
...
- Retiring the bespoke HTML risk renderer in `reporting/risk_html.py` in favor of a **headless UI snapshot** of the NiceGUI dashboard (Playwright). Planned subsequent changes: (a) add `market_helper/presentation/dashboard/snapshot.py` that launches the dashboard, waits for a sentinel DOM marker, inlines CSS/JS/Plotly assets, and writes a self-contained HTML file; (b) rewire CLI commands `risk-html-report` / `combined-html-report` to the snapshot pipeline with a `?snapshot=1` URL flag that hides the Actions console, Run History, and Refresh button; (c) delete the `render_html` / `render_risk_tab` / `_render_*` template code in `risk_html.py` (keeping the view-model builders intact) and the thin re-export shim in `market_helper/presentation/html/portfolio_risk_report.py`; (d) replace the HTML-string assertions in `tests/unit/reporting/test_risk_html.py` with view-model-level assertions.
- Remaining Risk Dashboard v1 polish: propagate `cm_sector` through `SecurityUniverseRow` → `SecurityReference` → `RiskInputRow` / `RiskMetricsRow` so the Commodity detail tab no longer re-reads `security_universe.csv` directly; add the Commodity cross-sector correlation heatmap using `pairwise_corr` on configured sector proxies (`commodity_sector_proxies` in `report_config.yaml`); add an Equity DM/EM summary block with two 100% stacked bars derived from `equity_country_policy_mix` + the country look-through.
