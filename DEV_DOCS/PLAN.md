...
- Added shared configs: `configs/regime_detection/regime_config.yml` and `configs/regime_detection/regime_policy.yml`.
...
### Completed
- **M1 FRED macro panel pipeline**: `market_helper/data_sources/fred/macro_panel.py` with per-series feather caches, publication-lag-aware forward-fill, `configs/regime_detection/fred_series.yml`, `scripts/run_fred_sync.sh`, and the `fred-macro-sync` CLI subcommand.
...
- **M4 Quadrant policy + orchestration + serialization**: `market_helper/suggest/quadrant_policy.py` (4-quadrant policy table + crisis overlay that redistributes `equity_shift_pct * intensity` of EQ to CASH/GOLD/FI and reduces vol multiplier), `configs/regime_detection/quadrant_policy.yml`, `MultiMethodRegimeSnapshot` dataclass with JSON roundtrip, and `market_helper/regimes/multi_method_service.py` orchestrator.
...
6. **P6 — Dashboard chrome alignment. [LANDED]**
   *Reviewed against P1–P4 reality:* the shared design tokens are already injected into the dashboard's `<head>` via `add_dashboard_styles()` (P1). The remaining seam was the visible chrome — `.pm-hero` slate-to-royal-blue gradient sitting around an iframe that now carries the unified tokens.
   What landed: dropped `.pm-hero` and replaced `_render_header` with a sticky `.pm-app-bar` mirroring the report's `<header class='app-bar'>` (brand pill + title + status meta + idle/running chip). Re-skinned `.pm-card`, `.pm-warning`, `.pm-error`, `.pm-loading`, `.pm-status-*` chip family, and `.pm-static-tab-button` to consume `var(--*)` tokens. Added a token-driven `.pm-status-card` primitive (replaces the legacy slate-50 `ui.card`-wrapped status tiles) so dashboard status surfaces match the report's `.metric` look. Iframe wrap dropped its outer `ui.card` border and uses a new `.pm-report-iframe` primitive with `var(--surface)` background, so the embedded report bleeds into the surrounding chrome with no visible seam. `render_action_card` flipped from `bg-slate-50` to the shared `.pm-card` primitive. `pm-status-*` chip semantics preserved; embedded report iframe still renders unchanged.
...
7. **P7 — Split operate from view. [LANDED]**
   *Reviewed against P1–P4 reality:* the state machinery (`_STALE_PAGE_CACHE`, `_render_*` functions in `pages/portfolio.py`) is layout-independent — moving the rendering call sites doesn't change state shape. We ended up with a custom slide-over `<aside class='pm-drawer'>` (toggled via `ui.run_javascript("document.body.classList.toggle('pm-drawer-open')")`) instead of `ui.right_drawer`, since the JS toggle is purely client-side and avoids a Python round-trip on every drawer open/close.
   What landed: app-bar gained a single primary **Refresh** button (`asyncio.create_task(run_action("refresh"))`) and an **Operate** chrome button that toggles `body.pm-drawer-open`. New `_render_operate_drawer` mounts a backdrop + `.pm-drawer` aside with sticky header, `×` close button, and four sections — Report Controls (the moved Recompute / Reload / Generate buttons via the new `_render_granular_buttons` helper), Pipeline Actions (`_render_action_console` — 5 action cards), Artifact Paths (the moved `Artifact Paths` expansion via the new `_render_artifact_paths_form` helper), and Progress Log (`_render_logs` without its outer card chrome). `_render_toolbar` gutted down to status cards + quick-link only — `/portfolio` first-paint is now app-bar + status row + main tabs (Report iframe / Artifacts metadata) with zero form inputs and zero file paths. State-cache round-trip preserved via the existing `@ui.refreshable` rendering pipeline; drawer content rebuilds on every refresh, but state bindings (`bind_value`, `state.action_statuses[...]`) keep their values intact.
   Concrete work:
...
**Post-P7 polish (cherry-picked from `design_mockup_v2.html`).**
- **[LANDED] Drift dumbbell.** `_render_policy_drift_chart` in [risk_html.py](market_helper/reporting/risk_html.py) now renders **● current** vs **○ target** dumbbells on a per-chart adaptive scale (0..max-weight). Right-side delta tag turns warn-amber at |drift| ≥ 2pp. Replaces the divergent-bar chart used in Asset Class / EQ Country / US Sector drift sections. Inline legend at the top of each chart. No data shape changes; `PolicyDriftRow` already carried `current_weight`/`policy_weight`/`active_weight`. Commit `cddf61a`.
- **[LANDED] SPY benchmark trace on cumulative perf chart.** New module [benchmark_history.py](market_helper/domain/portfolio_monitor/services/benchmark_history.py) extends `nav_cashflow_history.feather` with two columns (`bench_spy_return_usd`, `bench_spy_return_sgd`); SGD return is FX-compounded on the fly using the existing `fx_usdsgd_eod` column via `compound_returns_with_fx` (formula `(1 + r_usd) * (fx_t / fx_{t-1}) - 1`). Refresh path `refresh_benchmark_returns_in_history_feather` reuses the existing Yahoo `ensure_symbol_return_cache` (period=`max`, log-returns converted to simple via `expm1`); incremental thanks to the cache's freshness check. Plotly cumulative chart in [performance_html.py](market_helper/reporting/performance_html.py) gains a dotted SPY trace on percent mode (skipped on PnL mode). New CLI subcommand `benchmark-history-refresh --history-path PATH [--force]`. Backward-compat: old feathers without the columns load fine; if the column has no non-null rows for the active window, the trace is silently dropped.
