...
- Retiring the bespoke HTML risk renderer in `reporting/risk_html.py` in favor of a **headless UI snapshot** of the NiceGUI dashboard (Playwright). Phase A landed: `?snapshot=1` URL flag, `#snapshot-ready` sentinel, and a skeleton `market_helper/presentation/dashboard/snapshot.py` module. Remaining work: (a) add `playwright` to `env.yml` pip deps and run `playwright install chromium` from `scripts/setup_python_env.sh`; (b) inline external CSS/JS/Plotly assets into the captured HTML so the output is self-contained offline; (c) rewire CLI commands `risk-html-report` / `combined-html-report` in `market_helper/cli/main.py` to the snapshot pipeline; (d) delete the `render_html` / `render_risk_tab` / `_render_*` template code in `risk_html.py` (keeping the view-model builders intact), the thin re-export shim in `market_helper/presentation/html/portfolio_risk_report.py`, and the HTML half of `market_helper/reporting/combined_html.py`; (e) replace the HTML-string assertions in `tests/unit/reporting/test_risk_html.py` with view-model-level assertions.
- Remaining Risk Dashboard v1 polish: propagate `cm_sector` through `SecurityUniverseRow` → `SecurityReference` → `RiskInputRow` / `RiskMetricsRow` so the Commodity detail tab no longer re-reads `security_universe.csv` directly (currently worked around with a `@lru_cache`-backed reader in the dashboard panel).
