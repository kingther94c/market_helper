...
- **`macro_rules` method**: consumes the FRED macro panel from `fred_series.yml`, applies publication-lag-aware forward-fill to reduce lookahead, transforms each series into a comparable signal, then computes rolling z-scores. Growth inputs include PMI, unemployment change inverted, payrolls, industrial production, retail sales, and leading index. Inflation inputs include CPI/core CPI, PCE/core PCE, sticky CPI, 5y5y inflation expectations, and wages. Per-axis weighted averages are clipped by `zscore_clip` and smoothed by `min_consecutive_days` sign hysteresis.
- **`legacy_rulebook` method**: reuses the v1 market-stress rulebook from VIX/MOVE/HY OAS/Treasury/EQ/FI inputs. It maps the 7 native labels onto the 4 quadrants, preserves the native label for diagnostics, and emits the crisis flag/intensity from stress hysteresis. Key v1 thresholds: crisis enters at stress `0.75`, exits below `0.60`, inflationary rates threshold `0.20`, recovery window `20` days, and non-crisis persistence `5` days.
...
- **Policy resolution**: quadrant policy supplies base asset-class targets and vol multiplier. Crisis overlay shifts `equity_shift_pct * crisis_intensity` from EQ into CASH/GOLD/FI and reduces the vol multiplier. It intentionally avoids applying the legacy native 7-regime policy on top of the quadrant policy.
...
- **M6 HTML artifact + review notebook**: `regime-html-report` generates a standalone legacy/multi-method HTML artifact with ensemble quadrant, policy suggestion, method votes, recent history, and full-sample distribution; `notebooks/regime_detection/regime_v2_sanity_review.ipynb` provides a review path for historical checkpoint validation and HTML generation.
- **M7 Operator entry points**: `regime-refresh-report` refreshes stale source inputs (default 7-day freshness window), runs multi-method detection, and renders HTML; `regime-run-report` skips source refresh and reruns detection/reporting from existing local artifacts. `regime-input-sync` is available as a lower-level debugging command for the legacy market-stress JSON inputs.
...
- Regime v2 schema is covered by unit tests (`tests/unit/regimes/test_axes.py`, `test_macro_rules.py`, `test_ensemble.py`, `test_legacy_wrapper.py`, `test_quadrant_policy.py`, `test_multi_method_service.py`), reporting tests, and CLI e2e coverage. No manual 15-year backtest sanity check has been run yet (GFC / COVID / 2022 inflation) — use `notebooks/regime_detection/regime_v2_sanity_review.ipynb` once the FRED panel is synced on a host with a `FRED_API_KEY`.
