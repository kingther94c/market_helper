...
- Retiring the bespoke HTML risk renderer in `reporting/risk_html.py` in favor of a **headless UI snapshot** of the NiceGUI dashboard (Playwright). Phase A + B-1 + B-2 landed: `?snapshot=1` URL flag, `#snapshot-ready` sentinel, Playwright in `env.yml` / `setup_python_env.sh`, and `capture_snapshot()` now produces a fully self-contained offline HTML via the ossify-and-strip strategy (strip all scripts, inline CSS + font data URIs). Remaining work: (c) rewire CLI commands `risk-html-report` / `combined-html-report` in `market_helper/cli/main.py` to the snapshot pipeline — split into **c-risk** (risk-only, parity already in place) followed by a performance-tab parity fix and **c-combined**; (d) delete the `render_html` / `render_risk_tab` / `_render_*` template code in `risk_html.py` (keeping the view-model builders intact), the thin re-export shim in `market_helper/presentation/html/portfolio_risk_report.py`, and the HTML half of `market_helper/reporting/combined_html.py`; (e) replace the HTML-string assertions in `tests/unit/reporting/test_risk_html.py` with view-model-level assertions.
