...
### Outstanding
- NiceGUI does not yet expose a dedicated regime action; intended next step is to call the standalone HTML workflow from the GUI, similar to the performance/risk report actions.
...
1. **GUI action integration** — call `regime-html-report` from NiceGUI so regime HTML generation follows the same pattern as performance/risk actions.
